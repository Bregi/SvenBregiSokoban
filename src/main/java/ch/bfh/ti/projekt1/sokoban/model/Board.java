                ...
                if (isMoveAllowed(direction)) {
                    ...
                    if ((grid[position.getX()][position.getY() + 1].getState() == FieldState.DIAMOND)) {
                        if ((grid[position.getX()][position.getY() + 2].getState() == FieldState.GOAL)) {
                            grid[position.x][position.y + 2].setState(FieldState.COMPLETED);
                        } else {
                            grid[position.x][position.y + 2].setState(FieldState.DIAMOND);
                        }
                    }

                    if ((grid[position.getX()][position.getY() + 1].getState() == FieldState.COMPLETED)) {
                        //TODO: Player jetzt auf Goal, Diamant aus Ziel herausgeschoben
                        //TODO: wir brauchen noch einen weiteren Zustand (Player auf Goal)
                    }
                    grid[oldPosition.x][oldPosition.y].setState(FieldState.EMPTY);
                    grid[position.x][position.y + 1].setState(FieldState.PLAYER);
                ...
                if (isMoveAllowed(direction)) {
                    ...
                    if ((grid[position.getX() - 1][position.getY()].getState() == FieldState.DIAMOND)) {
                        if ((grid[position.getX() - 2][position.getY()].getState() == FieldState.GOAL)) {
                            grid[position.x - 2][position.y].setState(FieldState.COMPLETED);
                        } else {
                            grid[position.x - 2][position.y].setState(FieldState.DIAMOND);
                        }
                    }

                    if ((grid[position.getX() - 1][position.getY()].getState() == FieldState.COMPLETED)) {
                        //TODO: Player jetzt auf Goal, Diamant aus Ziel herausgeschoben
                        //TODO: wir brauchen noch einen weiteren Zustand (Player auf Goal)
                    }
                    ...
                    grid[oldPosition.x][oldPosition.y].setState(FieldState.EMPTY);
                    grid[position.x - 1][position.y].setState(FieldState.PLAYER);
